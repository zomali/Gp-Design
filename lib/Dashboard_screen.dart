...
class _Dashboard_screenState extends State<Dashboard_screen> {
  ...
  String TopicName = "Loading..";
  List<String> HighTopics = [];
  List<String> LowTopics = [];
  ...
  Widget build(BuildContext context) {
    addTOList();
    progress = (std.current_topic / 17) * 100;
    progress = progress.ceilToDouble();
    ...
    return Scaffold(
      ...
      body: Builder(builder: (context) {
        StudentCubit.get(context)
            .retriveTopicName(std.level, std.current_topic);
        StudentCubit.get(context).MaxTwoScoreTopics(int.parse(std.id));
        StudentCubit.get(context).MinTwoScoreTopics(int.parse(std.id));
        StudentCubit.get(context).getAllLevels(levelIDs);
        return BlocBuilder<StudentCubit, StudentState>(
          builder: (context, state) {
            if (state is StudentLoading) {
              ...
              var levelsList = studentCubit.allLevels;
              TopicName = studentCubit.TopicName;
              try {
                HighTopics = studentCubit.MaxTopicScore;
                if (HighTopics.length == 1) {
                  HighTopics.add("");
                ...
                if (HighTopics[0] == LowTopics[0] ||
                    HighTopics[1] == LowTopics[0]) {
                  LowTopics[0] = LowTopics[1];
                  LowTopics[1] = "";
                } else if (HighTopics[1] == LowTopics[1] ||
                    HighTopics[0] == LowTopics[1]) {
                  LowTopics[1] = "";
                }
              } catch (e) {
                HighTopics.add("Loading...");
                HighTopics.add("Loading...");
                LowTopics.add("Loading...");
                LowTopics.add("Loading...");
              }
    ...
    return Scaffold(
      ...
      body: StoryView(
        storyItems: [
          StoryItem.text(
            title:
                "I guess you'd love to see more of the system. That's great.",
            backgroundColor: Color(0xFF1565C0),
          ),
          StoryItem.text(
            title: "Nice!\n\nTap to continue.",
